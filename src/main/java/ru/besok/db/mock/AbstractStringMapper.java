...
public abstract class AbstractStringMapper implements StringMapper {
    ...
    @Override
    public StringBiFunction<Class<?>, ? extends Enum<?>> enumVal() {
        return (cl, v) -> {
            try {
                int el = Integer.parseInt(v);
                return (Enum<?>) cl.getEnumConstants()[el];
            } catch (NumberFormatException e) {
                return (Enum<?>) cl.getEnumConstants()[(int) getEnumByVal(cl, v)];
            }
        };
    }
}
